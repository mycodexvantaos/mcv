...
import type { WorkspaceSpec, WorkspaceStatus, WorkspacePhase, Resource, Membership, Role, QuotaSpec, QuotaUsage } from '../core/index';
...
export class WorkspaceService {
  ...
  async createWorkspace(input: {
    ...
    return {
      ...
      kind: 'workspace',
      metadata: {
        id: workspaceId, urn, kind: 'workspace', workspaceId,
        labels: { tier: input.tier ?? 'free' }, annotations: {},
        createdBy: input.ownerId, version: '1.0.0', resourceVersion: 1,
      ...
      spec: {
        ...
        settings: { dataResidency: 'auto', defaultLanguage: 'en', mfaRequired: false, retentionPolicy: { documents: 90, chatSessions: 90, auditEvents: 2555 } },
        quotas: defaultQuotas,
      },
      status: {
        phase: 'active', conditions: [{ type: 'Ready', status: 'True', reason: 'Created', message: 'Workspace created', lastTransitionTime: now }],
        memberCount: 1, resourceCount: 0, quotaUsage: { members: 1, resources: 0, collections: 0, modelEndpoints: 0, storageMb: 0 },
        suspended: false,
      },
    };
  }
  ...
  async checkQuota(workspaceId: string, resourceKind: string): Promise<{ allowed: boolean; current: number; limit: number }> {
    const workspace = await this.deps.database.queryFirst<{ quotas: string }>(
      'SELECT quotas FROM workspaces WHERE id = ?', [workspaceId]
    );
    ...
    const check = limitMap[resourceKind] ?? { current: usage.resources, limit: quotas.maxResources };
  ...
  private async getQuotaUsage(workspaceId: string): Promise<QuotaUsage> {
    const memberCount = await this.deps.database.queryFirst<{ count: number }>(
      'SELECT COUNT(*) as count FROM workspace_memberships WHERE workspace_id = ?', [workspaceId]
    );
    return {

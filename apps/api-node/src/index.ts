...
import { getEnvironmentConfig, loadEnvironmentConfig } from "@mycodexvantaos/core/config/environment";
...
function handleRequest(
  ...
  if (url === "/health/live") {
    ...
    res.end(JSON.stringify({ status: "ok", service: "mycodexvantaos-api", timestamp: new Date().toISOString() }));
    return;
  }

  if (url === "/health/ready") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ status: "ready", service: "mycodexvantaos-api", timestamp: new Date().toISOString() }));
    return;
  }

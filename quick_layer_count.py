...
layer_structure = {
    ...
    "Layer E (Deployment)": ["auto-scaler", "load-balancer", "ssl-manager"],
    "Layer F (Governance)": ["audit-logger", "compliance-checker", "policy-engine"],
}
...
overall_percentage = (
    (total_existing / total_components * 100) if total_components > 0 else 0
)
...
for layer_name, result in layer_results.items():
    ...
    print(
        f"{status} {layer_name}: {result['existing']}/{result['total']} ({result['percentage']:.0f}%)"
    )
    if result["missing"]:

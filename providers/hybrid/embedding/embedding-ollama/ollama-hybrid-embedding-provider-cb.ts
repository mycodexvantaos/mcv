...
this.baseUrl = cfg.baseUrl || 'http://localhost:11434';
  ...
  async embed(text: string): Promise<EmbedResult> {
    ...
    if (!this.isAvailable) {
      return { success: false, error: `OllamaHybridEmbeddingProvider not available. Use fallback: ${this.fallbackProviderId || 'native'}`, operationTime: Date.now() - startTime } as any;
    }
    try {

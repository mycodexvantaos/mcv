...
export class MongoDatabaseProvider extends CapabilityBase<MongoDatabaseConfig> {
  ...
  protected async doHealthCheck(): Promise<ProviderHealthCheckResult> {
    return { isHealthy: this.isMongoAvailable, status: this.isMongoAvailable ? ProviderHealthStatus.HEALTHY : ProviderHealthStatus.DEGRADED, checkTime: new Date().toISOString(), metrics: { hasFallback: !!this.fallbackProviderId } };
  }

...
export interface SearchResult { success: boolean; hits?: Record<string, unknown>[]; total?: number; error?: string; operationTime: number; }
export interface IndexResult { success: boolean; id?: string; error?: string; operationTime: number; }
...
this.node = cfg.node;
  ...
  async search(index: string, query: Record<string, unknown>): Promise<SearchResult> {
    ...
    if (!this.isAvailable) {
      return { success: false, error: `ElasticsearchProvider not available. Use fallback: ${this.fallbackProviderId || 'native'}`, operationTime: Date.now() - startTime } as any;
    }
    try {

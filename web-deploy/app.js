...
async function observerConnect() {
  ...
  if (!pairResult || !pairResult.paired) {
    ...
    await appendAudit(
      '觀察方已配對',
      `金鑰 ${key} 配對成功，Session: ${pairResult.session}`,
      'low'
    );
  }
...
async function subjectConfirmAndVanish() {
  ...
  await appendAudit(
    '被觀察方確認',
    `金鑰 ${S.generatedKey.slice(0, 4)}-**** 已確認，準備部署靜默服務`,
    'info'
  );
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      document.body.innerHTML =
        '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0a0d17;color:#1e2340;font-family:sans-serif;font-size:12px;">系統守護服務已在背景運行</div>';
...
function navMonitor(screen) {
  ...
  document.querySelectorAll('.mscreen').forEach((s) => {
    s.classList.add('hidden');
    s.classList.remove('active');
  });
  ...
  document
    ...
    .forEach((b) => b.classList.toggle('active', b.dataset.mnav === screen));
...
function updateEngineUI(running) {
  ...
  if (running) {
    dot.className = 'w-1.5 h-1.5 rounded-full active';
    dot.id = 'monitor-engine-dot';
    label.textContent = '運行中';
    label.className = 'text-threat-low';
    statE.textContent = '●';
    statE.className = 'stat-value text-threat-low';
  } else {
...
function enrollMonitorDevice() {
  ...
  const d = {
    id: `DEV-${String(S.devices.length + 1).padStart(3, '0')}`,
    name: names[i],
    type: types[i],
    os: osOpts[i],
    status: 'online',
...
function renderDevicesList() {
  ...
  if (!c) return;
  if (!S.devices.length) {
    c.innerHTML = '<div class="text-gray-600 text-center py-8">尚未連線裝置</div>';
    return;
  }
  c.innerHTML = S.devices
    .map(
      (d) => `
...
function updateDeviceSelector() {
  ...
  sel.innerHTML =
    ...
    S.devices
      ...
      .map((d) => `<option value="${d.id}">${d.type} ${d.name}</option>`)
      .join('');
}
...
function simFeedEvent() {
  ...
  S.stats.totalEvents++;
  S.feedRate = (S.stats.totalEvents / ((Date.now() - (S._startTime || Date.now())) / 1000)).toFixed(
    1
  );
...
const THREAT_TPL = [
  {
    title: '線上賭場存取偵測',
    desc: '使用者存取 bet365.com — 線上博弈平台。24小時內多次造訪。',
    severity: 'critical',
    category: 'gambling',
    action: '網址封鎖',
  },
...
function simThreatDetection() {
  ...
  if (t.severity === 'critical' || t.severity === 'high')
    showToast(`${t.title} — ${d.name}`, t.severity);
...
function renderThreatTimeline() {
  ...
  if (!filtered.length) {
    c.innerHTML =
      '<div class="text-gray-600 text-center py-12 text-xs">尚未偵測到威脅 — 系統安全</div>';
    return;
  }
...
function ackThreat(id) {
  ...
  if (t) {
    t.acknowledged = true;
    renderThreatTimeline();
    appendAudit('威脅確認', `威脅 ${id} 已確認`);
  }
}

function sevLabel(s) {
  return { critical: '嚴重', high: '高危', medium: '中等', low: '低', info: '資訊' }[s] || s;
}
function catLabel(c) {
  return (
    {
...
function updateDashboard() {
  ...
  if (el('m-stat-sessions')) {
    S.stats.sessions = S.engineRunning ? S.devices.filter((d) => d.status === 'online').length : 0;
    el('m-stat-sessions').textContent = S.stats.sessions;
  }
...
function updatePosture() {
  ...
  let score = 100;
  if (total > 0)
    score = Math.max(0, 100 - S.stats.critical * 15 - S.stats.high * 8 - S.stats.medium * 3);
  ...
  if (labelEl) {
    let l = '安全';
    if (score < 40) l = '危險';
    else if (score < 60) l = '升高';
    else if (score < 80) l = '中等';
...
function toggleMonitorCapture() {
  ...
  if (S.captureActive) {
    ...
    btn.classList.remove('bg-sentinel-600', 'hover:bg-sentinel-700');
    btn.classList.add('bg-threat-critical', 'hover:bg-red-700');
    view.classList.add('active-capture');
    view.innerHTML =
      '<div class="w-full h-full flex items-center justify-center"><div class="text-center"><div class="text-4xl mb-3">📡</div><div class="text-xs text-threat-low font-bold">靜默擷取運行中</div><div class="text-[10px] text-gray-500 mt-1">目標裝置無任何可見指示</div><div class="text-[10px] text-gray-600 mt-2">AES-256 加密串流</div><div class="mt-3"><div class="spinner mx-auto"></div></div></div></div>';
    status.textContent = '擷取運行中 — 加密串流';
    status.className = 'text-[10px] text-threat-low';
    appendAudit('擷取啟動', '靜默螢幕擷取已啟動');
  } else {
    btn.textContent = '啟動靜默擷取';
    btn.classList.add('bg-sentinel-600', 'hover:bg-sentinel-700');
    btn.classList.remove('bg-threat-critical', 'hover:bg-red-700');
    view.classList.remove('active-capture');
    view.innerHTML =
      '<div class="text-center"><div class="text-4xl mb-2 opacity-30">🖥️</div><div>靜默擷取未啟動</div><div class="text-[10px] mt-1">目標裝置無任何可見指示</div></div>';
    status.textContent = '未啟動擷取';
...
function simTelemetry() {
  ...
  c.insertAdjacentHTML(
    'afterbegin',
    `<div class="feed-entry feed-info"><span class="text-gray-600">${new Date().toLocaleTimeString()}</span> <span class="text-sentinel-400">${lines[Math.floor(Math.random() * lines.length)]}</span></div>`
  );
...
function generateAnomalyData() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    S.anomalyScores.push({
      date: d.toLocaleDateString('zh', { weekday: 'short' }),
      score: Math.random() * 0.35 + 0.05,
    });
  }
}

function renderBehaviorScreen() {
  renderBaseline();
  renderAnomalyChart();
...
function renderBaseline() {
  ...
  if (!devs.length) {
    c.innerHTML = '<div class="text-gray-600 text-center py-8 text-xs">正在分析行為模式…</div>';
    return;
  }
  const metrics = [
    {
      ...
      icon: '⏱',
      vals: devs.map(
        () => `${Math.floor(Math.random() * 4 + 1)}時${Math.floor(Math.random() * 60)}分`
      ),
    },
    {
      label: '尖峰活動時段',
      icon: '🌙',
      vals: devs.map(
        () => `${String(Math.floor(Math.random() * 5 + 20) % 24).padStart(2, '0')}:00`
      ),
    },
    {
      label: '異常分數',
      icon: '📊',
      vals: devs.map(() => (Math.random() * 0.4 + 0.05).toFixed(3)),
    },
    {
      label: '威脅暴露',
      icon: '⚠',
      vals: devs.map(() => ['低', '中等', '升高'][Math.floor(Math.random() * 3)]),
    },
    {
      label: '行為偏移',
      icon: '📈',
      vals: devs.map(() => `${(Math.random() * 12 - 6).toFixed(1)}%`),
    },
  ];
  c.innerHTML = metrics
    .map(
      (m) =>
        `<div class="baseline-metric"><div class="text-xs text-gray-400">${m.icon} ${m.label}</div><div class="text-xs font-bold">${m.vals.map((v) => `<span class="text-sentinel-300">${v}</span>`).join(' · ')}</div></div>`
    )
    .join('');
}
...
function renderAnomalyChart() {
  ...
  data.forEach((d, i) => {
    ...
    ctx.fillStyle = d.score > 0.5 ? '#ef4444' : '#818cf8';
    ctx.fill();
    ctx.fillStyle = '#64748b';
    ctx.font = '9px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(d.date, x, H - 10);
    ctx.fillText(d.score.toFixed(2), x, y - 10);
  });
}
...
function renderRiskIndicators() {
  ...
  c.innerHTML = inds
    .map(
      (i) =>
        `<div class="risk-bar-container"><span class="text-[11px] min-w-[140px] text-gray-400">${i.label}</span><div class="risk-bar-bg"><div class="risk-bar-fill" style="width:${i.val}%;background:${i.color}"></div></div><span class="text-[11px] font-bold min-w-[32px] text-right" style="color:${i.color}">${i.val}%</span></div>`
    )
    .join('');
}
...
function addMonitorRule() {
  ...
  S.rules.push(r);
  renderRules();
  showToast(`規則「${r.name}」已建立`, 'info');
  appendAudit('規則建立', `規則「${r.name}」已加入引擎`);
}
...
function toggleRule(id) {
  ...
  if (r) {
    r.active = !r.active;
    renderRules();
    appendAudit('規則切換', `規則「${r.name}」${r.active ? '啟用' : '停用'}`);
  }
}
function deleteRule(id) {
  ...
  if (r) {
    ...
    renderRules();
    showToast(`規則「${r.name}」已刪除`, 'medium');
    appendAudit('規則刪除', `規則「${r.name}」已移除`);
  }
}
...
function updateChainInfo() {
  const e = document.getElementById('m-chain-entries'),
    h = document.getElementById('m-chain-hash'),
    s = document.getElementById('m-chain-status');
...
function renderAuditLog() {
  const c = document.getElementById('m-audit-log');
  if (!c) return;
  if (!S.auditChain.length) {
    c.innerHTML = '<div class="text-gray-600 text-center py-8">無審計紀錄</div>';
    return;
  }
  c.innerHTML = '';
  S.auditChain
...
function toggleMonitorSetting(key) {
  ...
  if (btn) {
    btn.className = `toggle-btn ${S.settings[key] ? 'on' : 'off'}`;
    btn.textContent = S.settings[key] ? '開' : '關';
  }
...
function toggleMonitorModule(key) {
  ...
  if (btn) {
    btn.className = `toggle-btn ${S.modules[key] ? 'on' : 'off'}`;
    btn.textContent = S.modules[key] ? '開' : '關';
  }
  appendAudit('模組切換', `${catLabel(key)}偵測器 ${S.modules[key] ? '啟動' : '停用'}`);
  showToast(
    `${catLabel(key)}偵測器 ${S.modules[key] ? '啟動' : '停用'}`,
    S.modules[key] ? 'low' : 'medium'
  );
}
...
function toggleTheme() {
  S.isDarkTheme = !S.isDarkTheme;
  document.body.classList.toggle('light-theme', !S.isDarkTheme);
}

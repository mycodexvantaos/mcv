...
const DelegateCodingTaskInputSchema = z.object({
  taskDescription: z
    .string()
    .describe(
      'A clear description of the coding task to be performed by the AI agent (e.g., "generate a React button component", "refactor this function to be more readable", "write unit tests for this class").'
    ),
  codeContext: z
    .string()
    .optional()
...
const DelegateCodingTaskOutputSchema = z.object({
  ...
  agentRole: z
    .string()
    .describe(
      'The specific role the AI agent adopted for this task (e.g., "Code Generator", "Refactoring Assistant", "Test Writer").'
    ),
});

...
const COMMON_WORDS = new Set([
  ...
  'may', 'might', 'must', 'shall', 'should', 'need', 'very', 'still', 'much', 'more',
  'where', 'why', 'while', 'each', 'too', 'own', 'such', 'same', 'those', 'through',
]);
...
export function extractFeatures(sentence: string): SentenceFeatures {
  ...
  const clauseMarkers = (sentence.match(/\b(and|but|or|however|although|because|since|while|which|that|where|when)\b/gi) || []).length;
  ...
  const transitionWords = (sentence.match(/\b(however|therefore|furthermore|moreover|consequently|nevertheless|additionally|subsequently|thus|hence|accordingly)\b/gi) || []).length;

# Self-Hosted Quickstart: v0.1.0-rc.1

This guide walks through running MyCodeXvantaOS v0.1.0-rc.1 locally or on a self-hosted server using Docker.
...
```bash
docker build -t mycodexvantaos:v0.1.0-rc.1 .
```
...
# Expected: version matching v0.1.0-rc.1
...
```bash
./scripts/smoke/docker-smoke.sh mycodexvantaos:v0.1.0-rc.1
```

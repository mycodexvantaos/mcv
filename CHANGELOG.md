...
- **CI (P0)**: Restored all push- and pull_request-triggered GitHub Actions workflows on `main`. Repository policy `sha_pinning_required` was silently toggled to `true` between commits `63936ff` (16:00 UTC, all CI green) and `98c7353` (16:11 UTC, all CI red on the same workflow files). With SHA pinning enforced, every `uses: ...@v4` tag reference produced `startup_failure` at workflow boot. Set `sha_pinning_required` back to `false` via repository Actions permissions API; `allowed_actions: selected`, `github_owned_allowed: true`, and `verified_allowed: true` remain enforced. SHA-pinning of all third-party actions is tracked as governed deferred work for v0.2.0 (FOLLOW-UP-1..4 in ADR-0013). See `docs/adr/adr-0013-disable-sha-pinning-required-policy.md`. No workflow file content modified; no dependencies, runtime, Dockerfile, Node/pnpm versions, package manager, CI architecture, or release identity changed. Task type: F + C. Priority: P0.

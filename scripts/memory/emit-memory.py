...
def emit_memory(
    content: str, tags: List[str], entities: List[str], mem_type: str = "observation"
):
    mem_id = (
        f"urn:mycodexvantaos:memory:{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
    )
    item = MemoryItem(
        memory_id=mem_id,
        content=content,
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python3 emit-memory.py <content> <tags_comma_separated> <entities_comma_separated>"
        )
        sys.exit(1)

...
def main():
    ...
    for pkg_name, cov in sorted(results.items()):
        print(
            f"{pkg_name:<30} {cov['statements']:>7.1f}% {cov['branches']:>7.1f}% {cov['functions']:>7.1f}% {cov['lines']:>7.1f}%"
        )
        if cov["statements"] > 0:
    ...
    if count > 0:
        ...
        print(
            f"{'AVERAGE':<30} {avg_stmts:>7.1f}% {avg_branches:>7.1f}% {avg_functions:>7.1f}% {avg_lines:>7.1f}%"
        )

...
class ResultAnalyzer:
    ...
    async def analyze(self, result: Dict[str, Any], actual_outcome: str) -> Dict[str, Any]:
...
class AdaptiveOptimizer:
    ...
    async def optimize(self, metrics: Dict[str, Any], current_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if avg_accuracy < 0.7:
            optimized['evidence_sufficiency_threshold'] = max(0.5, optimized.get('evidence_sufficiency_threshold', 0.7) - 0.05)
            optimized['hypothesis_confidence_min'] = max(0.4, optimized.get('hypothesis_confidence_min', 0.6) - 0.05)
        elif avg_accuracy > 0.9:
            optimized['evidence_sufficiency_threshold'] = min(0.95, optimized.get('evidence_sufficiency_threshold', 0.7) + 0.05)
            optimized['hypothesis_confidence_min'] = min(0.9, optimized.get('hypothesis_confidence_min', 0.6) + 0.05)
...
class FeedbackLoop:
    ...
    async def process_feedback(self, decision_id: str, actual_outcome: str, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Feedback recorded: {decision_id} - accuracy={analysis['accuracy']:.0%}")

...
Returns the number of milliseconds since the Unix _Epoch_ (1 January, 1970 UTC) as a number. All invocations of `$millis()` within an evaluation of an expression will all return the same value.
...
Convert the `number` representing milliseconds since the Unix _Epoch_ (1 January, 1970 UTC) to a formatted string representation of the timestamp as specified by the `picture` string.
...
Convert a `timestamp` string to the number of milliseconds since the Unix _Epoch_ (1 January, 1970 UTC) as a number.

...
function getDeployProvider() {
    ...
    const external = argoToken ? new deploy_argocd_1.ExternalDeployProvider('/path/to/kubeconfig', argoToken) : null;
    ...
    switch (mode) {
        ...
        case 'connected':
            if (!external)
                throw new Error('Connected mode requires ARGOCD_AUTH_TOKEN');
            return external;
        case 'hybrid':
        case 'auto':

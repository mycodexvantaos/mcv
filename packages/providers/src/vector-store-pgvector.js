...
class ConnectedPgVectorProvider {
    ...
    async initialize() { }
    async healthCheck() { return { status: 'down', reason: 'Postgres DB unreachable' }; }
    async shutdown() { }
    async storeEmbedding(id, text, vector) { throw new Error("PG Down"); }
    async searchSimilar(vector, topK) { throw new Error("PG Down"); }
}

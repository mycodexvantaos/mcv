...
class ConnectedAuthProvider {
    ...
    async initialize() { }
    async healthCheck() { return this.isOnline ? { status: 'healthy' } : { status: 'down', reason: 'IDP unreachable' }; }
    async shutdown() { }
    async verifyToken(token) { if (!this.isOnline)
        throw new Error("IDP Down"); return true; }
}

...
const DECISION_TEMPLATES = [
  {
    ...
    domain: 'ai',
    description:
      'Guides selection of optimal LLM provider based on cost, latency, context window, and capability requirements.',
    factors: ['cost', 'latency', 'context-window', 'capability', 'data-residency'],
    status: 'published',
  },
  {
    id: 'dec-002',
    ...
    domain: 'data',
    description:
      'Decision framework for choosing between PostgreSQL, MongoDB, Redis, and vector databases.',
    factors: ['query-pattern', 'scale', 'consistency', 'latency', 'cost'],
    status: 'published',
  },
  {
    id: 'dec-003',
    ...
    domain: 'devops',
    description:
      'Guides choice between rolling, blue-green, canary, and feature-flag deployment strategies.',
    factors: ['risk-tolerance', 'rollback-speed', 'traffic-volume', 'team-size'],
    status: 'published',
  },
  {
    id: 'dec-004',
    ...
    domain: 'security',
    description:
      'Framework for selecting between Keycloak, Supabase Auth, and custom JWT implementations.',
    factors: ['compliance', 'sso-requirements', 'user-volume', 'self-hosted'],
    status: 'draft',
  },
];
...
export async function POST(req: NextRequest) {
  ...
  if (!context || !domain) {
    return NextResponse.json(
      { success: false, error: 'context and domain are required' },
      { status: 400 }
    );
  }
  ...
  const guide = {
    ...
    recommendation: {
      ...
      tradeoffs: [
        {
          option: 'Primary',
          pros: ['Lower cost', 'Better latency'],

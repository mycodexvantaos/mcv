...
export class EventRouterService {
  ...
  async addRule(rule: Omit<RoutingRule, 'id' | 'createdAt'>): Promise<RoutingRule> {
    ...
    this.providers.observability.info('Routing rule added', {
      id,
      source: rule.sourceTopic,
      target: rule.targetTopic,
    });
    return entry;
  }
  ...
  async listRules(): Promise<RoutingRule[]> {
    if (this.rules.size === 0) {
      const result = await this.providers.stateStore.scan<RoutingRule>({
        pattern: 'eventrouter:rule:*',
      });
  ...
  async route(sourceTopic: string, event: unknown): Promise<string[]> {
    const matching = Array.from(this.rules.values()).filter(
      (r) => r.enabled && r.sourceTopic === sourceTopic
    );
    const routed: string[] = [];
    ...
    if (routed.length > 0)
      this.providers.observability.debug('Event routed', { source: sourceTopic, targets: routed });
    return routed;
  }

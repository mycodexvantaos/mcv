...
export interface RAGResult {
  answer: string;
  sources: Array<{ documentId: string; content: string; score: number }>;
  latency: number;
}
...
export class RAGService {
  ...
  async indexDocument(
    ...
    // Store document
    await this.providers.storage.put(`rag/documents/${id}.json`, JSON.stringify(doc), {
      contentType: 'application/json',
    });
    await this.providers.stateStore.set(`ai:rag:doc:${id}`, {
      id,
      metadata,
      indexedAt: doc.indexedAt,
    });
    // Store in database for search
    await this.providers.database.execute(
      'INSERT OR REPLACE INTO rag_documents (id, content, metadata, indexed_at) VALUES (?, ?, ?, ?)',
  ...
  async query(request: RAGQuery): Promise<RAGResult> {
    ...
    const searchResult = await this.providers.database.query<{ id: string; content: string }>(
      'SELECT id, content FROM rag_documents ORDER BY indexed_at DESC LIMIT ?',
      [request.topK ?? 5]
    );
    const sources = searchResult.rows.map((row, idx) => ({
      documentId: row.id,
  ...
  async listDocuments(
    limit: number = 50
  ): Promise<Array<{ id: string; metadata: Record<string, unknown>; indexedAt: number }>> {
    const result = await this.providers.stateStore.scan<{
      id: string;
      metadata: Record<string, unknown>;
      indexedAt: number;
    }>({ pattern: 'ai:rag:doc:*', count: limit });

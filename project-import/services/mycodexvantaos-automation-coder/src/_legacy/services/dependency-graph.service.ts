...
export interface DepEdge {
  from: string;
  to: string;
  type: 'imports' | 'depends' | 'extends' | 'implements';
}
export interface DepGraph {
  nodes: DepNode[];
  edges: DepEdge[];
  cycles: string[][];
  stats: { nodeCount: number; edgeCount: number; cycleCount: number; maxDepth: number };
}
...
export class DependencyGraphService {
  ...
  async buildGraph(repoName: string): Promise<DepGraph> {
    ...
    this.providers.observability.info('Dependency graph built', {
      repo: repoName,
      nodes: nodes.length,
      edges: edges.length,
    });
  ...
  async getGraph(repoName: string): Promise<DepGraph | null> {
    return (
      (await this.providers.stateStore.get<DepGraph>(`decon:graph:${repoName}`))?.value ?? null
    );
  }
  ...
  private calculateMaxDepth(nodes: DepNode[], edges: DepEdge[]): number {
    ...
    const getDepth = (node: string, visited: Set<string>): number => {
      ...
      const depth =
        children.length === 0
          ? 0
          : 1 + Math.max(...children.map((c) => getDepth(c, new Set(visited))));

...
export class StateMachineService {
  ...
  async createInstance(
    ...
    const def = (
      await this.providers.stateStore.get<StateMachineDef>(`automation:sm:def:${definitionId}`)
    )?.value;
  ...
  async transition(instanceId: string, event: string): Promise<StateMachineInstance> {
    const instance = (
      await this.providers.stateStore.get<StateMachineInstance>(
        `automation:sm:instance:${instanceId}`
      )
    )?.value;
    if (!instance) throw new Error(`Instance not found: ${instanceId}`);
    const def = (
      await this.providers.stateStore.get<StateMachineDef>(
        `automation:sm:def:${instance.definitionId}`
      )
    )?.value;
    ...
    if (!nextState)
      throw new Error(`No transition for event "${event}" from state "${instance.currentState}"`);
    ...
    this.providers.observability.debug('State transition', {
      instanceId,
      event,
      from: instance.history[instance.history.length - 1].from,
      to: nextState,
    });
    return instance;
  }

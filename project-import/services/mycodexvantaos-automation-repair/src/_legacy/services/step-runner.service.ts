...
export class StepRunnerService {
  ...
  async run(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        await this.providers.stateStore.set(
          `automation:step:${stepName}:${execution.startedAt}`,
          execution
        );
        ...
        if (attempt < maxAttempts) {
          ...
          this.providers.observability.warn('Step retrying', {
            step: stepName,
            attempt,
    ...
    this.providers.observability.error('Step failed after all retries', {
      step: stepName,
  ...
  async getHistory(stepName: string, limit: number = 10): Promise<StepExecution[]> {
    const result = await this.providers.stateStore.scan<StepExecution>({
      pattern: `automation:step:${stepName}:*`,
      count: limit,
    });

...
export class WorkflowEngineService {
  ...
  async execute(workflowId: string, input?: Record<string, unknown>): Promise<WorkflowRun> {
    const def = (
      await this.providers.stateStore.get<WorkflowDef>(`automation:workflow:${workflowId}`)
    )?.value;
  ...
  async getRun(runId: string): Promise<WorkflowRun | null> {
    return (
      (await this.providers.stateStore.get<WorkflowRun>(`automation:run:${runId}`))?.value ?? null
    );
  }
  ...
  async listWorkflows(): Promise<WorkflowDef[]> {
    const result = await this.providers.stateStore.scan<WorkflowDef>({
      pattern: 'automation:workflow:*',
      count: 100,
    });
  ...
  private async executeStep(step: WFStep, input?: Record<string, unknown>): Promise<unknown> {
    switch (step.type) {
      case 'action':
        await this.providers.queue.enqueue('automation:actions', {
          step: step.name,
          config: step.config,
          input,
        });
      ...
      case 'wait': {
        ...
        const ms =
          typeof rawMs === 'number' && Number.isFinite(rawMs)
            ? Math.max(0, Math.min(rawMs, 3_600_000))
            : 1000;
        await new Promise((r) => setTimeout(r, ms));
        return { waited: true };
      }
      case 'condition':
        return { evaluated: true, result: step.config['default'] ?? true };
      default:

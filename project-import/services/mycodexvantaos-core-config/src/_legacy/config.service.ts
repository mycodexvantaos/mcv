...
export interface ConfigEntry {
  key: string;
  value: unknown;
  scope: 'global' | 'service' | 'environment';
  namespace?: string;
  version: number;
  updatedAt: number;
  updatedBy?: string;
...
export class ConfigService {
  ...
  async get<T = unknown>(
    ...
    const scopes: Array<'environment' | 'service' | 'global'> = options?.scope
      ? [options.scope]
    ...
    for (const scope of scopes) {
      ...
      if (entry) {
        this.cache.set(cacheKey, entry.value);
  ...
  private buildKey(key: string, scope?: string, namespace?: string): string {
    return `${scope ?? 'global'}:${namespace ?? 'default'}:${key}`;
  }
}

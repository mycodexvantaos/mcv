...
export class FeatureFlagService {
  ...
  async isEnabled(
    flagName: string,
    context?: { userId?: string; group?: string }
  ): Promise<boolean> {
  ...
  async upsert(flag: Partial<FeatureFlag> & { name: string }): Promise<FeatureFlag> {
    ...
    this.providers.observability.info('Feature flag updated', {
      flag: flag.name,
  ...
  async listFlags(): Promise<FeatureFlag[]> {
    const result = await this.providers.stateStore.scan<FeatureFlag>({
      pattern: 'featureflag:*',
      count: 200,
    });
  ...
  async deleteFlag(name: string): Promise<boolean> {
    return this.providers.stateStore.delete(`featureflag:${name}`);
  }

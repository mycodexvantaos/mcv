...
export interface LBConfig {
  name: string;
  strategy: LBStrategy;
  endpoints: Array<{ host: string; port: number; weight?: number }>;
...
export class LoadBalancerService {
  ...
  async configure(config: LBConfig): Promise<void> {
    ...
    this.providers.observability.info('Load balancer configured', {
      name: config.name,
  ...
  async resolve(name: string): Promise<{ host: string; port: number } | null> {
    ...
    switch (config.strategy) {
      ...
      case 'random':
        return config.endpoints[randomInt(config.endpoints.length)];
      case 'least-connections': {
        let min = Infinity;
        let selected = config.endpoints[0];
        for (const ep of config.endpoints) {
  ...
  async getConfig(name: string): Promise<LBConfig | null> {

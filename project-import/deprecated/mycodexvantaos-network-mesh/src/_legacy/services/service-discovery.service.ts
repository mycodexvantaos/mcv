...
export class ServiceDiscoveryService {
  ...
  async register(endpoint: Omit<ServiceEndpoint, 'registeredAt'>): Promise<ServiceEndpoint> {
    ...
    await this.providers.stateStore.set(
      `mesh:endpoint:${endpoint.name}:${endpoint.host}:${endpoint.port}`,
      entry,
      { ttl: 300 }
    );
    this.providers.observability.info('Service endpoint registered', {
      name: endpoint.name,
      host: endpoint.host,
      port: endpoint.port,
    });
  ...
  async discover(name: string, onlyHealthy: boolean = true): Promise<ServiceEndpoint[]> {
    const result = await this.providers.stateStore.scan<ServiceEndpoint>({
      pattern: `mesh:endpoint:${name}:*`,
    });

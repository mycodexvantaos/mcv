...
export class IngestionService {
  ...
  async ingest(source: SourceType, config: Record<string, unknown>): Promise<IngestionJob> {
    const job: IngestionJob = {
      id: `ingest-${Date.now()}-${randomBytes(3).toString('hex').slice(0, 6)}`,
      source,
      config,
      status: 'running',
      recordsProcessed: 0,
    ...
    try {
      switch (source) {
        case 'file':
          job.recordsProcessed = await this.ingestFromFile(config);
          break;
        case 'api':
          job.recordsProcessed = await this.ingestFromAPI(config);
          break;
        case 'database':
          job.recordsProcessed = await this.ingestFromDB(config);
          break;
        default:
  ...
  async listJobs(limit: number = 20): Promise<IngestionJob[]> {
    const result = await this.providers.stateStore.scan<IngestionJob>({
  ...
  private async ingestFromAPI(config: Record<string, unknown>): Promise<number> {
    ...
    for (const record of data) {
      await this.providers.queue.enqueue('pipeline:raw-data', { source: 'api', data: record });
    }
    return data.length;
  }
  ...
  private async ingestFromDB(config: Record<string, unknown>): Promise<number> {
    ...
    for (const row of result.rows) {
      await this.providers.queue.enqueue('pipeline:raw-data', { source: 'database', data: row });
    }

...
export class TransformationService {
  ...
  async transform(data: unknown[], steps: TransformStep[]): Promise<unknown[]> {
    ...
    for (const step of steps) {
      ...
      switch (step.type) {
        case 'map':
          result = result.map((item) => this.applyMapping(item, step.config));
          break;
        case 'filter':
          result = result.filter((item) => this.applyFilter(item, step.config));
          break;
        case 'aggregate':
          result = [this.applyAggregate(result, step.config)];
          break;
        case 'flatten':
          result = result.flat((step.config['depth'] as number) ?? 1);
          break;
        default:
          break;
      }
      this.providers.observability.debug('Transform step applied', {
        step: step.name,
        before,
        after: result.length,
      });
  ...
  private applyFilter(item: unknown, config: Record<string, unknown>): boolean {
    ...
    for (const [key, expected] of Object.entries(conditions)) {
      if ((item as any)[key] !== expected) return false;
    }
    return true;
  }
  ...
  private applyAggregate(items: unknown[], config: Record<string, unknown>): unknown {
    ...
    switch (op) {
      ...
      case 'sum':
        return {
          sum: items.reduce((acc: number, i) => acc + (Number((i as any)?.[field]) || 0), 0),
        };
      case 'avg': {
        const vals = items.map((i) => Number((i as any)?.[field]) || 0);

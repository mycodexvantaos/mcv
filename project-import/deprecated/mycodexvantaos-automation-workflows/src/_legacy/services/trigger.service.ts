...
export class TriggerService {
  ...
  async create(
    trigger: Omit<TriggerDef, 'id' | 'createdAt' | 'triggerCount'>
  ): Promise<TriggerDef> {
    ...
    this.providers.observability.info('Trigger created', {
      id,
      type: trigger.type,
  ...
  async fire(triggerId: string, payload?: Record<string, unknown>): Promise<void> {
    const trigger = (
      await this.providers.stateStore.get<TriggerDef>(`workflows:trigger:${triggerId}`)
    )?.value;
    ...
    await this.providers.queue.enqueue('workflows:trigger:fired', {
      triggerId,
      workflowId: trigger.workflowId,
      payload,
      timestamp: Date.now(),
    });
    this.providers.observability.info('Trigger fired', {
      triggerId,
      type: trigger.type,
      workflow: trigger.workflowId,
    });
  ...
  async get(triggerId: string): Promise<TriggerDef | null> {
    return (
      (await this.providers.stateStore.get<TriggerDef>(`workflows:trigger:${triggerId}`))?.value ??
      null
    );
  }
  async delete(triggerId: string): Promise<boolean> {
    return this.providers.stateStore.delete(`workflows:trigger:${triggerId}`);
  }
  async enable(triggerId: string): Promise<TriggerDef> {
    return this.toggle(triggerId, true);
  }
  async disable(triggerId: string): Promise<TriggerDef> {
    return this.toggle(triggerId, false);
  }

  async list(type?: TriggerType): Promise<TriggerDef[]> {
    const result = await this.providers.stateStore.scan<TriggerDef>({
  ...
  private async toggle(id: string, enabled: boolean): Promise<TriggerDef> {
    const trigger = (await this.providers.stateStore.get<TriggerDef>(`workflows:trigger:${id}`))
      ?.value;

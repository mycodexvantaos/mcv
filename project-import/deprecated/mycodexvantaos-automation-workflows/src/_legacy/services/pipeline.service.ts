...
export class PipelineService {
  ...
  async get(pipelineId: string): Promise<Pipeline | null> {
    return (
      (await this.providers.stateStore.get<Pipeline>(`workflows:pipeline:${pipelineId}`))?.value ??
      null
    );
  }
  ...
  async updateStage(
    ...
    if (pipeline.stages.every((s) => s.status === 'succeeded')) {
      pipeline.status = 'succeeded';
      pipeline.completedAt = Date.now();
    } else if (pipeline.stages.some((s) => s.status === 'failed')) {
  ...
  async list(repo?: string, limit: number = 20): Promise<Pipeline[]> {
    const result = await this.providers.stateStore.scan<Pipeline>({
      pattern: 'workflows:pipeline:*',
      count: limit,
    });

...
export class AlertingService {
  ...
  async upsertRule(rule: AlertRule): Promise<AlertRule> {
    if (this.providers.observability.upsertAlertRule)
      return this.providers.observability.upsertAlertRule(rule);
    await this.providers.stateStore.set(`alert:rule:${rule.id}`, rule);
    return rule;
  }

  async listRules(): Promise<AlertRule[]> {
    if (this.providers.observability.listAlertRules)
      return this.providers.observability.listAlertRules();
  ...
  async deleteRule(ruleId: string): Promise<void> {
    if (this.providers.observability.deleteAlertRule)
      return this.providers.observability.deleteAlertRule(ruleId);
  ...
  async listAlerts(options?: {
    ...
    if (this.providers.observability.listAlerts)
      return this.providers.observability.listAlerts(options);
    return [];
  }

  async acknowledge(alertId: string, acknowledgedBy: string): Promise<void> {
    if (this.providers.observability.acknowledgeAlert)
      return this.providers.observability.acknowledgeAlert(alertId, acknowledgedBy);
  ...
  async fireAlert(opts: {
    ...
    await this.providers.notification.send({
      ...
      subject: `[${opts.severity.toUpperCase()}] ${opts.ruleName}`,
      body: opts.message,
      recipients: ['ops-team'],
    });

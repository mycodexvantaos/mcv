import { LoggingService, MetricsService, TracingService, AlertingService, bootstrap } from '../src';

describe('LoggingService', () => {
  let svc: LoggingService;
  beforeEach(() => {
    svc = new LoggingService('test-svc');
  });
...
describe('TracingService', () => {
  ...
  beforeEach(() => {
    svc = new TracingService('test-tracer');
  });
  ...
  it('should build a trace from spans', () => {
    ...
    const child = svc.startSpan('child', {
      parentContext: { traceId: parent.traceId, spanId: parent.id },
    });
    svc.endSpan(child.id);
    svc.endSpan(parent.id);

...
export class SchedulerService {
  ...
  async schedule(
    ...
    await this.providers.queue.enqueue('scheduler:tasks', task, {
      delay,
      maxAttempts: task.maxAttempts,
    });
    await this.providers.stateStore.set(`scheduler:task:${task.id}`, task);
    this.providers.observability.info('Task scheduled', {
      taskId: task.id,
      name,
  ...
  async listTasks(status?: TaskStatus, limit: number = 50): Promise<ScheduledTask[]> {
    const result = await this.providers.stateStore.scan<ScheduledTask>({

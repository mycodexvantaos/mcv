...
export class RegistryService {
  ...
  async register(
    service: Omit<RegisteredService, 'registeredAt' | 'lastHeartbeat' | 'healthy'>
    ...
    this.providers.observability.info('Service registered', {
  ...
  async lookup(name: string): Promise<RegisteredService | null> {
    const entry = await this.providers.stateStore.get<RegisteredService>(
  ...
  async list(filter?: {
    ...
    const result = await this.providers.stateStore.scan<RegisteredService>({
    ...
    if (filter) {
      ...
      if (filter.healthy !== undefined)
        services = services.filter((s) => s.healthy === filter.healthy);
    }
    return services;
  }

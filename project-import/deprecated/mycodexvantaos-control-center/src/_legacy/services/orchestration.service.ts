...
export class OrchestrationService {
  ...
  async execute(
    ...
    this.providers.observability.info('Orchestration started', {
      runId,
      action,
    ...
    await this.providers.stateStore.set(`orchestration:run:${runId}`, run);
    this.providers.observability.info('Orchestration completed', {
      runId,
      status: run.status,
  ...
  async getRunStatus(runId: string): Promise<OrchestrationRun | null> {
    const entry = await this.providers.stateStore.get<OrchestrationRun>(
  ...
  async listRuns(limit: number = 20): Promise<OrchestrationRun[]> {
    const result = await this.providers.stateStore.scan<OrchestrationRun>({
      pattern: 'orchestration:run:*',
      count: limit,
    });
  ...
  private async executeForRepo(action: OrchAction, repo: string): Promise<void> {
    switch (action) {
      case 'sync':
        await this.providers.repo.listBranches(repo);
        break;
      case 'validate':
        await this.providers.validation.validate({ type: 'repository', path: repo });
        break;
      case 'deploy':
        await this.providers.deploy.deploy({
          repoName: repo,
          ref: 'main',
        ...
        break;
      case 'healthcheck':
        await this.providers.repo.getRepo(repo);
        break;
      case 'rollback':
        await this.providers.deploy.rollback(repo, 'production');
        break;
    }
  }
}

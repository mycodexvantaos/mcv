...
export interface RepoState {
  ...
  lastDeploy?: number;
  currentRef: string;
  status: 'synced' | 'pending' | 'deploying' | 'error';
...
export class StateTrackingService {
  ...
  async updateState(updates: Partial<PlatformState>): Promise<PlatformState> {
    ...
    this.providers.observability.info('Platform state updated', {
  ...
  async updateRepoState(name: string, updates: Partial<RepoState>): Promise<RepoState> {
    ...
    const current = state.repositories[name] ?? {
      name,
      lastSync: 0,
      currentRef: 'main',
  ...
  async checkpoint(label?: string): Promise<string> {
    ...
    await this.providers.stateStore.set(
      `platform:checkpoint:${checkpointId}`,
      { ...state, checkpointLabel: label ?? checkpointId },
      { ttl: 86400 * 30 }
    );

...
class DependencyResolver:
    ...
    def get_tier_order(self, repositories: List[str]) -> Dict[str, int]:
        ...
        def calculate_tier(repo: str) -> int:
            ...
            max_dep_tier = max(
                calculate_tier(dep) for dep in deps if dep in repositories
            )
...
def main():
    ...
    parser.add_argument(
        "--target-planes", default="", help="Comma-separated list of target planes"
    )
    parser.add_argument(
        "--repositories",
        default="",
    ...
    parser.add_argument(
        "--output", default="execution_order.json", help="Output file path"
    )
    ...
    # Parse arguments
    target_planes = (
        [p.strip() for p in args.target_planes.split(",")]
        if args.target_planes
        else None
    )
    repositories = (
    ...
    try:
        order = resolver.resolve_execution_order(
            action=args.action, target_planes=target_planes, repositories=repositories
        )
        ...
        for i, repo in enumerate(order["execution_order"], 1):
            print(
                f"  {i}. {repo['name']} (Plane: {repo['plane']}, Tier: {repo['tier']}, Priority: {repo['priority']})"
            )

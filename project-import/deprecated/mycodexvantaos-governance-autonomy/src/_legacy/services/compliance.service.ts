...
export class ComplianceService {
  ...
  async runCheck(framework: string, target: string): Promise<ComplianceReport> {
    ...
    for (const rule of rules) {
      ...
      checks.push({
        id: `check-${Date.now()}-${randomBytes(3).toString('hex').slice(0, 6)}`,
        framework,
        rule: rule.name,
    ...
    const overallStatus: ComplianceStatus =
      score === 100 ? 'compliant' : score >= 80 ? 'warning' : 'non_compliant';
    ...
    this.providers.observability.info('Compliance check completed', {
      framework,
      score,
  ...
  async getReport(reportId: string): Promise<ComplianceReport | null> {
    return (
      (await this.providers.stateStore.get<ComplianceReport>(`governance:compliance:${reportId}`))
        ?.value ?? null
    );
  }

  async listReports(framework?: string): Promise<ComplianceReport[]> {
    const result = await this.providers.stateStore.scan<ComplianceReport>({
  ...
  private async getRules(framework: string): Promise<Array<{ name: string; description: string }>> {
    ...
    return (
      defaults[framework] ?? [{ name: 'generic-check', description: 'Generic compliance check' }]
    );
  }
  ...
  private async evaluateRule(rule: { name: string }, target: string): Promise<ComplianceStatus> {
    ...
    try {
      const entry = await this.providers.stateStore.get(
        `compliance:evidence:${target}:${rule.name}`
      );

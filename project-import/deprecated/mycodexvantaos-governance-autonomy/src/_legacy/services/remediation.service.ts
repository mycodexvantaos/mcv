...
export class RemediationService {
  ...
  async remediate(
    violation: string,
    options?: { automated?: boolean; dryRun?: boolean }
    ...
    this.providers.observability.info('Remediation action', {
      id,
      violation,
  ...
  async listActions(status?: RemediationStatus): Promise<RemediationAction[]> {
    const result = await this.providers.stateStore.scan<RemediationAction>({
      pattern: 'governance:remediation:*',
      count: 100,
    });
  ...
  async getAction(actionId: string): Promise<RemediationAction | null> {
    return (
      (await this.providers.stateStore.get<RemediationAction>(`governance:remediation:${actionId}`))
        ?.value ?? null
    );
  }
}

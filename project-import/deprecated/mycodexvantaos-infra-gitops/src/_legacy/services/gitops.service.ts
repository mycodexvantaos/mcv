...
export class GitOpsService {
  ...
  async getSpec(repo: string, environment: string): Promise<DeploymentSpec | null> {
    const entry = await this.providers.stateStore.get<DeploymentSpec>(
      `gitops:spec:${repo}:${environment}`
    );
    return entry?.value ?? null;
  }

  async listSpecs(): Promise<DeploymentSpec[]> {
    const result = await this.providers.stateStore.scan<DeploymentSpec>({
      pattern: 'gitops:spec:*',
    });

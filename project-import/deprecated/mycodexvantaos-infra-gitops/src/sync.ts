  ...
  async detectDrift(): Promise<DriftReport> {
    ...
    for (const manifest of this.desiredState.manifests as k8s.KubernetesObject[]) {
      ...
      if (manifest.kind === 'Deployment') {
        const desiredImage = (manifest as k8s.V1Deployment).spec?.template.spec?.containers?.[0]
          ?.image;
        if (desiredImage && desiredImage !== live.image) {
          drifts.push({
            resource: key,
            field: 'spec.template.spec.containers[0].image',
            expected: desiredImage,
            actual: live.image ?? null,
          });
        }
      }
    }

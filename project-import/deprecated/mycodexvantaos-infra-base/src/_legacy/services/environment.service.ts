...
export class EnvironmentService {
  ...
  async upsert(env: Partial<EnvironmentDef> & { name: string }): Promise<EnvironmentDef> {
    ...
    const entry: EnvironmentDef = {
      name: env.name,
      displayName: env.displayName ?? existing?.displayName ?? env.name,
      type: env.type ?? existing?.type ?? 'development',
      variables: env.variables ?? existing?.variables ?? {},
      secrets: env.secrets ?? existing?.secrets ?? [],
      protections: env.protections ?? existing?.protections ?? [],
      active: env.active ?? existing?.active ?? true,
  ...
  async delete(name: string): Promise<boolean> {
    return this.providers.stateStore.delete(`infra:env:${name}`);
  }
